...
- **README preloader docs**: document the exo event-log divergence failure mode (staggered pod restarts; `Nack attempt` log loops; launches that never materialize) with the verified all-at-once restart remedy and a pointer to [exo-rkllama#37](https://github.com/freed-dev-llc/exo-rkllama/issues/37) ([#85](https://github.com/freed-dev-llc/turing-rk1-cluster/pull/85)).
...
- **Preloader CronJob died on a flaky exo API** - once rk1-2 returned and the CronJob unwedged, reconcile jobs failed back-to-back: a timed-out `/state` fetch made `loaded_nodes()` emit an empty operand (busybox ash: `sh: out of range`, reproduced in alpine:3.20), and under `set -eu` a failed `placement=$(curl ...)` assignment killed a job with a one-line log. The script now falls back to empty state on fetch failures, guards the tool install, the placement/launch calls, and the response parsing (log and defer to the next run), defers when no node identities are visible, and its `MODEL` default matches the live CronJob target (`deepseek-r1-distill-qwen-14b-rkllm`, changed out-of-band since #74), so `kubectl apply` no longer flips the deployed model ([#84](https://github.com/freed-dev-llc/turing-rk1-cluster/pull/84)).
